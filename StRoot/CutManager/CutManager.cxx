...
bool CutManager::passTrackPhi(StPicoTrack *track, float dca)
{
    if(!track) return kFALSE;
    ...
    if(dca > ConstManager::mDcaTrMax_phi)
    {
        return kFALSE;
    }

    // primary pt and momentum cut: PtMin = 0.1
    if(!(track->pMom().Perp() > ConstManager::mGlobPtMin && track->pMom().Mag() < ConstManager::mPrimMomMax))
    {
        return kFALSE;
    }
...
//---------------------------------------------------------------------------------
// PID
Float_t CutManager::getMass2(StPicoTrack *track, StPicoDst *pico)
{
    Float_t Mass2 = -999.0;
...
bool CutManager::passSigKaonCut(StPicoTrack* track, Float_t scale_nSigma_factor)
{
    Float_t nSigmaKaon = track->nSigmaKaon();
    //if(fabs(nSigmaKaon*scale_nSigma_factor) > TriFlow::mNSigmaKaonMax)
    if( (nSigmaKaon < mConfigs.nSig_ka_low) &&
         (nSigmaKaon > mConfigs.nSig_ka_high) )
    {
        return kFALSE;
    }
    return kTRUE;
